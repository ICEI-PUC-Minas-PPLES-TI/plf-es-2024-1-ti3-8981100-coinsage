...
def get_last(db: Session) -> Optional[analysis.Analysis]:
    return (
        ...
        .order_by(analysis.Analysis.date.desc())
        .where(analysis.Analysis.ended == True)
        .first()
    )
...
def update_ended(db: Session, uuid_value: Uuid):
    db.query(analysis.Analysis).filter(analysis.Analysis.uuid == uuid_value).update({analysis.Analysis.ended: True})
    db.commit()


def delete(db: Session, uuid_value: Uuid):
    db.query(analysis.Analysis).filter(analysis.Analysis.uuid == uuid_value).delete()
    db.commit()
